...
* [x] A5 合并
* [x] reference 指标无明显回归（merge 后 `reference_residual_rmse ≈ 7.21e-08`，与 A2 历史基线逐位一致）
* [ ] 相关结论写入 `PROJECT_STATE.md`（A4 / 文档线下一轮同步补齐 lap_tau_mode + rel_l2 审计数值）
...
* 状态：`INTEGRATING`
* Owner：`A0 / Codex`
* 最近更新：`2026-04-19` 已合并并推送 A1、A2、A3、A5 到 `int/multi-agent`；集成 smoke（pytest 131 passed / 1-epoch train / evaluate_run with reference，`reference_residual_rmse ≈ 7.21e-08`）全部通过
...
* 状态：`MERGED`
* Owner：`A5 / Codex`
* 最近更新：`2026-04-19` A5 已完成 DoD；3 提交（`feat(physics): add lap_tau audit mode`、`test(physics): add residual audit coverage`、`docs: add A5 handoffs`）合入 `int/multi-agent`；默认 `mixed_legacy` 对 A2 reference solver 行为逐元素等价，`stretched_divergence` 仅作审计候选保留（rel_l2_eval≈0.508 / rel_l2_pml≈0.522，self-loss~3e-15，cross-loss~6e-4；非纯 PML 局部修正）；保留为 programmatic-only mode，未暴露 YAML/CLI 入口
