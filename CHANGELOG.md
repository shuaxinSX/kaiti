...
- `training.supervision.target_kind` 默认锁定为 `scattering_envelope`，并拒绝误接 `reference_wavefield.npy` 作为 data loss 标签。
- 训练评估产物新增 A3 相位重构误差预算指标：`phase_tau_error_budget_h2` / `phase_tau_error_budget_h` 及对应波场误差预算。
- 训练评估产物新增 A4 容量诊断：全波场 Nyquist 模式下界、FNO 模式比、Neumann 强散射代理与有限深度尾项代理。
