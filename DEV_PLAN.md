...
> 如需修改决策，先改文档，再开分支重做。
>
> 里程碑进度勾选以 `int/multi-agent @ b80dc88` 为事实依据。
> 完整的能力清单与测试对应关系见 [PROJECT_STATE.md §3 / §7](PROJECT_STATE.md)。
> 第 3 节冻结决策保持原貌，不随基线更新。
...
**检查项**：
- [x] 能安装依赖（`pip install -e ".[dev]"`；`requirements.txt` 已退化为兼容壳）
- [x] `pytest` 能跑（`tests/test_bootstrap.py` 等 16 个测试文件）
- [x] 配置能读（`src/config.load_config` + `tests/test_config.py` 覆盖多 overlay）
- [x] 日志能输出（`src/train/runner.configure_logging`）
- [x] `import src` 不报错
...
**检查项**：
- [x] 网格 shape 正确（含 PML 扩展）— `tests/test_grid.py`
- [x] 坐标系定义统一（i↔y, j↔x 不混淆）
- [x] 源点索引和物理坐标双向转换一致
- [x] 距离场在源点处为 eps（不为 0）
- [x] PML 内介质退化为 s₀ — `tests/test_medium.py`
- [x] 3 个手写 case 对上（均匀介质、线性梯度、含PML边界）
- [x] 可视化正常（`run_training` 会写 `field_heatmaps.png`）
...
**检查项**：
- [x] 远离震源时 τ₀ 趋势合理（线性增长）
- [x] ∇τ₀ 模长 ≡ s₀（常数，允许浮点误差 < 1e-12）
- [x] u₀ 远场渐近行为：`|u₀| ∝ 1/√r`（2D柱面扩散）— `tests/test_background.py`
- [x] 震源点无 NaN/Inf 泄漏（`r_safe = clamp(r, eps)`）
- [x] u₀ 可视化：实部呈同心环振荡（`wavefield.png`）
...
- [x] 因果律检验不遗漏
- [x] 无明显假波纹
- [x] 收敛轮次合理（debug 配置 `fsm_max_iter=20` 已足够）

**通过标准**：
- [x] 常速介质中 `max|α − 1| < 1e-8` — `tests/test_eikonal.py`
- [x] 平滑变速介质中 τ 连续、无突变
- [x] 无 NaN
- [x] 可视化 α 场平滑
- [ ] 与参考 FMM（如 scikit-fmm）结果对比误差 < 1e-4（尚未接入 scikit-fmm，暂以内部一致性为准）
...
**检查项**：
- [x] 不直接对 τ 做二阶差分（`src/physics/tau_ops.py` 走链式法则）
- [x] 链式法则重组正确 — `tests/test_tau_ops.py`
- [x] 震源附近 Δτ 数值可控（不出现异常尖峰）
- [x] 常速介质中 `|∇τ|² ≈ s₀²`（程函一致性由测试覆盖）
...
**检查项**：
- [x] 差分核不使用 autograd（`src/physics/diff_ops.py` 全走 `F.conv2d`）— `tests/test_diff_ops.py`
- [x] γ' 用解析式（不做数值差分）
- [x] PML 层只在边界起作用，内部 γ≡1
- [x] 内部区域 A=1, B=0，退化为标准中心差分
- [x] 卷积核 shape 和 padding 正确（不错位）

**通过标准**：
- [x] 平面波 `e^{ikx}` 进入 PML 后衰减（可视化）
- [x] 衰减后振幅低于机器精度（由 `residual_heatmaps.png` log10 图辅助验证）
- [x] 无明显边界反射 — `tests/test_pml.py`
...
**检查项**：
- [x] 震源点不出现 `0 * inf`（数值稳定）
- [x] PML 内 RHS 为 0（由介质退化保证）
- [x] mask 半径写进配置文件（`loss.source_mask_radius`）
- [x] π/4 相位补偿正确应用 — `tests/test_rhs.py`
- [x] RHS 在全空间有界

**通过标准**：
- [x] 数值稳定（无 NaN/Inf）
- [x] RHS 可视化连续、震源处归零
- [x] 常速介质中 RHS ≡ 0（A2 参考解 homogeneous 产出零包络已复核）
...
- [x] 通道维度统一（8 → hidden → 2）
- [x] 实虚双通道一致
- [x] 前向不爆显存（debug 网格 CPU smoke 验证）
- [x] 零填充逻辑正确
- [x] 输出层零初始化

**通过标准**：
- [x] 前向可跑通（无报错）
- [x] 输出 shape 正确 `[B, 2, H, W]` — `tests/test_model_forward.py`
- [x] 零初始化时输出全零（无散射初态）
- [x] 显存占用合理
...
**检查项**：
- [x] 物理域内残差有限（无 NaN/Inf）
- [x] PML 行为合理（残差在边界衰减）
- [x] 震源附近 mask 生效（掩码区残差为 0）— `tests/test_residual.py`
- [x] 阻尼池解耦正确（物理区内该项 ≡ 0）
- [x] ω² 归一化生效
...
- [x] 全图无 NaN/Inf
- [x] 小样例 forward 成功
- [x] 常速介质 + 零初始化网络 → 残差 = −RHS_eq/ω²（纯源项）
- [x] 可视化残差分布合理（`residual_heatmaps.png` 六宫格含 log10 尺度）
...
**检查项**：
- [x] loss 能下降（`tests/test_train.py` 覆盖三种模式）
- [x] 不出现数值爆炸
- [x] 输出图像合理（`wavefield.png` + `field_heatmaps.png`）
- [x] 常速介质：Â_scat ≈ 0（A2 homogeneous 参考解产出零包络）
- [x] 变速介质：Â_scat 非零且平滑
...
**监督扩展（A3 增量，本版本合入）**：
- PDE-only / data-only / hybrid 三种损失模式通过 `training.lambda_pde` + `training.lambda_data` + `training.supervision.{enabled, reference_path}` 声明
- `reference_envelope.npy`（由 `scripts/solve_reference.py` 生成）作为监督标签
- 详见 `PROJECT_STATE.md §3.3` 与 `handoffs/A3_to_A4.txt`
...
| 编号 | 问题 | 当前结论 | 状态 |
|------|------|---------|------|
| Q1 | α 求解器最终选 FSM 还是 FMM | FSM（`src/physics/eikonal.py`） | ✅ 已冻结 |
| Q2 | source mask 半径取 1.0h / 1.5h / 2.0h | 1.5h（`loss.source_mask_radius=1.5`） | ✅ 已冻结 |
| Q3 | PML 厚度默认值 | 20 格点（base）/ 5 格点（debug） | ✅ 已冻结 |
| Q4 | σ_max 取值 | 按反射系数公式计算（`pml.R0=1e-6`、`pml.power=2`） | ✅ 已冻结 |
| Q5 | FNO 保留模式数 | 16（base）/ 8（debug） | ✅ 已冻结 |
| Q6 | NSNO Block 数 | 4（base）/ 2（debug） | ✅ 已冻结 |
| Q7 | 是否需要焦散面 Δτ 高斯软化 | 当前 debug/smooth_lens 不需要 | 🟡 A5 复核 |
| Q8 | 复数激活函数选型 | GELU（实虚分开激活，`src/models/nsno.py`） | ✅ 已冻结 |
| Q9 | 课程学习频率调度策略 | 线性递增 ω（待正式训练引入） | ⏸ A6 实验 |
| Q10 | 输入特征是否需要 Z-score 归一化 | 网络输入可以，PDE Loss 系数禁止 | ⏸ A6 实验 |
