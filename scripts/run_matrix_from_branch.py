"""
Launch the large A6 matrix from batch specs stored on another git branch.

This script stays on the current integrated branch and only reads the matrix
YAML definitions from `spec_branch`. It does not call the branch-provided
launcher.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the full matrix from branch-stored batch specs.")
    parser.add_argument(
        "--spec-branch",
        default="exp/benchmark-matrix",
        help="Branch that stores the authoritative B*.yaml batch specs.",
    )
    parser.add_argument(
        "--base-config",
    ...
    parser.add_argument(
        "--output-root",
        default="outputs/matrix_full_from_branch",
...
def discover_batch_specs(repo_root: Path, spec_branch: str) -> List[Dict[str, Any]]:
    files_output = git_output(repo_root, "ls-tree", "-r", "--name-only", spec_branch)
    paths = [
        line.strip()
        for line in files_output.splitlines()
        if line.strip().startswith("configs/experiments/B") and line.strip().endswith(".yaml")
    ]

    specs: List[Dict[str, Any]] = []
    for rel_path in sorted(paths):
        text = git_output(repo_root, "show", f"{spec_branch}:{rel_path}")
        spec = yaml.safe_load(text) or {}
        spec["__file__"] = rel_path
        specs.append(spec)
    return specs
...
def build_manifest_rows(
    ...
    batch_specs = discover_batch_specs(repo_root, args.spec_branch)
