...
def resolve_config_path(manifest_row: Dict[str, str], run_dir: Path) -> Optional[Path]:
    candidates = [run_dir / "config_merged.yaml"]
    effective_path = manifest_row.get("effective_config_path")
    if effective_path:
        candidates.append(Path(effective_path))
...
def collect_run_entries(manifest_rows: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    for row in manifest_rows:
        ...
        config_path = resolve_config_path(row, run_dir)
        if (run_dir / "summary.json").exists() and config_path is not None:
            entries.append({"manifest": row, "run_dir": run_dir, "config_path": config_path})
    return entries
...
def compute_final_loss_components(run_dir: Path, cfg: Dict[str, Any], config_path: Path) -> Dict[str, Any]:
...
def build_report_row(
    ...
    row.update(derive_axes(cfg, entry["config_path"]))
    ...
    row.update(
        {
            ...
            "phase_tau_error_budget_h2": maybe_float(summary.get("phase_tau_error_budget_h2", metrics.get("phase_tau_error_budget_h2"))),
            "phase_tau_error_budget_h": maybe_float(summary.get("phase_tau_error_budget_h", metrics.get("phase_tau_error_budget_h"))),
            "wavefield_phase_error_budget_mean_h2": maybe_float(summary.get("wavefield_phase_error_budget_mean_h2", metrics.get("wavefield_phase_error_budget_mean_h2"))),
            "wavefield_phase_error_budget_p95_h2": maybe_float(summary.get("wavefield_phase_error_budget_p95_h2", metrics.get("wavefield_phase_error_budget_p95_h2"))),
            "wavefield_phase_error_budget_mean_h": maybe_float(summary.get("wavefield_phase_error_budget_mean_h", metrics.get("wavefield_phase_error_budget_mean_h"))),
            "wavefield_phase_error_budget_p95_h": maybe_float(summary.get("wavefield_phase_error_budget_p95_h", metrics.get("wavefield_phase_error_budget_p95_h"))),
            "full_wave_nyquist_mode_floor": maybe_float(summary.get("full_wave_nyquist_mode_floor", metrics.get("full_wave_nyquist_mode_floor"))),
            "fno_mode_to_full_wave_nyquist": maybe_float(summary.get("fno_mode_to_full_wave_nyquist", metrics.get("fno_mode_to_full_wave_nyquist"))),
            "scattering_strength_proxy": maybe_float(summary.get("scattering_strength_proxy", metrics.get("scattering_strength_proxy"))),
            "neumann_proxy_convergent": maybe_bool(summary.get("neumann_proxy_convergent", metrics.get("neumann_proxy_convergent"))),
            "neumann_depth_tail_proxy": maybe_float(summary.get("neumann_depth_tail_proxy", metrics.get("neumann_depth_tail_proxy"))),
...
def build_report_assets(output_root: Path) -> Dict[str, Any]:
    ...
    for entry in collect_run_entries(manifest_rows):
        run_dir = entry["run_dir"]
        cfg = load_yaml(entry["config_path"])
        ...
        final_components = compute_final_loss_components(run_dir, cfg, entry["config_path"])
    ...
    failure_rows = normalize_failure_rows(
        read_dict_rows(meta_root / "failures.csv") + read_jsonl_rows(meta_root / "failures.jsonl")
    )
    return {
